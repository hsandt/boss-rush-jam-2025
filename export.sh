...
game_title_case="Spin Knight"
all_platforms=(windows macos linux web)
godot_bin="godot4.3_stable"
...
export_platform_release() {
  ...
  case "$platform" in
    ...
    macos )
      preset="macOS"
      platform_zip_name="macOS"
      target="${game_title_case}.zip"
      ;;
    web )
      preset="Web"
      platform_zip_name="Web"
      target="index.html"
      ;;
    * )
  ...
  subfolder="$game_title_case v$version - $platform_zip_name"
  folder_path="$version_folder/$game_title_case v$version - $platform_zip_name"
  ...
  pushd "$version_folder"
    ...
    if [[ "$platform" == "macos" ]]; then
      ...
      cp "${subfolder}/${game_title_case}.zip" "${subfolder}.zip"
    else
      echo "Zipping..."
